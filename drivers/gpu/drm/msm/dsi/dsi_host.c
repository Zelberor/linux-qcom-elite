...
static void dsi_update_dsc_timing(struct msm_dsi_host *msm_host, bool is_cmd_mode)
{
	...
	bytes_per_pkt = dsc->slice_chunk_size * msm_host->dsc_slice_per_pkt; /* * slice_per_pkt; */
	 ...
	 * pkt_per_line will be equal to slice per intf for now. //FIXME
	 */
...
static void dsi_timing_setup(struct msm_dsi_host *msm_host, bool is_bonded_dsi)
{
	...
	if (msm_host->mode_flags & MIPI_DSI_MODE_VIDEO) {
		...
		if (!msm_host->dsc)
			 ...
			 *       and adjust DSC math to account for slice_per_pkt. //FIXME
			 */
